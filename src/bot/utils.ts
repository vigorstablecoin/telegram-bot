...
export const getGroupsToNotify = async (notificationLevel: NOTIFICATION_LEVEL) => {
  return Group.find({
    dacNotificationLevel: MoreThanOrEqual(notificationLevel)
  });
};
...
export const sendToAll = async (notificationLevel: NOTIFICATION_LEVEL, message, messageOptions?) => {
  ...
  for (const toNotify of [...users, ...groups]) {
    // https://core.telegram.org/bots/api#sendmessage
    try {
      await bot.telegram.sendMessage(toNotify.telegramChatId, message, messageOptions);
    } catch (error) {
      // ignore 403: Forbidden: bot was blocked by the user 
      if(error.code !== 403) {
        logger.error(error.message)
